...
class AmzSelectionConfirmationTests(unittest.TestCase):
    ...
    def test_build_card_has_prices_quantities_links_and_decision_buttons(self):
        ...
        self.assertIn("📦 建议采购：", rendered)
        self.assertIn("💶 竞品价：€25.99", rendered)
        self.assertIn("🏷️ 建议价：", rendered)
        self.assertIn("📈 月销依据", rendered)
        self.assertIn("样本ASIN", rendered)
        self.assertIn("近似竞品：", rendered)
        self.assertIn("类目新品：", rendered)
        self.assertIn("样本 1 个", rendered)
        self.assertIn("数据可信度", rendered)
        self.assertNotIn("竞品售价、月销来源、建议售价与各站采购量", rendered)
    ...
    def test_site_snapshot_fallback_populates_non_de_price_and_margin(self):
        ...
        self.assertIn("🇬🇧 UK 英国：竞品价 £28.45", summary)
        self.assertIn("中企最佳 C 148.2/57.9%", summary)
        self.assertIn("样本ASIN 不可用", summary)
        self.assertIn("近似竞品月销", summary)
        self.assertIn("类目新品月销", summary)
        self.assertIn("样本 1 个", summary)
