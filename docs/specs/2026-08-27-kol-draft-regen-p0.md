...
1. KOL 专用 Key 缺失时，`/health` 明确显示 degraded，`POST /draft/regen` 不启动后台生成并返回 503；同时登记 60 秒失败去重记录，使飞书重复回调静默，不重复发红卡/文字。
...
4. 后台 job 成功、业务失败或直接抛异常后，使用聪哥分身3号 PATCH 原卡为明确终态；只有新版已建且至少一张可操作的 App3 私聊审核卡拿到真实 `message_id` 才能显示绿色成功，群信息卡不算完成。PATCH 失败重试一次，再给原操作人发送 App3 兜底结果卡；失败卡保留同 App 的重试按钮。缺 Key 的即时失败卡同样具备两次 PATCH、App3 兜底和重试按钮。
