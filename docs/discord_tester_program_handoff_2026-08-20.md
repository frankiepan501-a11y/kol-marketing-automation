...
|---|---|---|---|
| 公开报名 | Discord 两步 10 题 Modal、硬门槛、自动分配测试路线、客观预评分、写报名台账 | 发布公告、看边界项 | Discord 返回报名编号，台账有同一用户记录 |
...
4. 员工账号完整走一次两步 10 题报名；Discord 先显示提交中，再返回报名编号。
5. 飞书报名台账出现测试记录，敏感地址/电话字段为空。
   - 新版字段：`喜爱游戏IP`、`配件关注点`。
   - 旧问卷字段 `Amazon购买品类`、`断连问题回答`、`功能测试回答`、`申请理由` 在新版重填时应清空。
