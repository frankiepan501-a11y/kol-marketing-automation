...
状态：G2 进行中；生产表结构和只读证据开关已上线，参与记录写入与所有真实外联保持关闭。
...
- 活动专项：`test_launch_*.py` 67/67 全部通过（另有 3 个子测试通过）。
- 全仓：288 项中 287 项通过（另有 3 个子测试通过）；唯一失败为本次未改动的 `test_zeabur_watchdog.ZeaburWatchdogTests.test_run_once_alerts_any_project_service_failed_deployment`。该失败与 G1 前一致。
- `py_compile` 通过。
...
2. G2：已获批准并执行中。表结构、证据开关和 NYXI 非官方推定规则已完成；剩余 Dave 活动证据配置和只读排序回读。
...
- 当前部署 commit：`6d684bb430bd809c0fed717bd5720e7e728286d4`；NYXI 非官方规则完成后将生成下一部署。
- 参与表 ID：`tblt0zD4hDb7sFqn`。
- 新增字段：活动 19、节点 9、参与记录 26，均已回读验证。
- Dave 证据配置版本、排序版本：待本轮只读测试写入后回读。
- Dave 首个名单版本：未写生产。
