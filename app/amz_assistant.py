...
async def handle_feishu_callback(payload: dict[str, Any]) -> dict:
    ...
    if action.startswith("amz_proc_review_"):
        from . import amz_procurement_preview

        return await amz_procurement_preview.handle_callback(_card_event(payload))
    ...
    if action.startswith("amz_selection_"):
        from . import amz_selection_confirmation

        return await amz_selection_confirmation.handle_callback(_card_event(payload))
