...
- model: DeepSeek Chat (env DTC_WEEKLY_DEEPSEEK_API_KEY)
...
- ⭐ 双框架 KPI 独立 callout: Frankie 升级率 / 独立站运营专员自决率 / 首响合规率 / 退款 ≤$100 自批比例
  - 目标: 升级率 < 20%, 自决率 > 80%, 首响合规 > 95%

## 11. SEO 自动化产能 + 选题池消费率 ⭐ 新增
- 6 工作流执行情况 (本周次数/成功/失败/失败原因)
- 内容产出: SEO 新闻稿目标 12 篇/周 (周一至周六双站各 1, PK 6 + FL 6)
  + SEO 商业意图 2 篇/周 (周二/周四双站轮转), 总计 14 篇/周
- 选题池状态 (按状态分布/新增/消费/低库存预警 < 10)
- 实际产出 vs 目标差距分析 (用 expected_per_week 字段对比)
...
async def _call_llm(system_prompt: str, user_prompt: str, max_tokens: int = 8000) -> str:
    ...
    api_key = os.environ.get("DTC_WEEKLY_DEEPSEEK_API_KEY", "").strip()
    if not api_key:
        raise RuntimeError("DTC_WEEKLY_DEEPSEEK_API_KEY env 未设")
