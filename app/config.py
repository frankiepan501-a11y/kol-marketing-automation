...
KOL_ENDPOINT_ALERT_DEDUP_PATH = env(
    "KOL_ENDPOINT_ALERT_DEDUP_PATH", "/data/kol_endpoint_alert_dedup.sqlite3"
)
...
T_UPLOAD_INTAKE = env("T_UPLOAD_INTAKE", "tblKwm9Ock8ObRgf")
...
YOUTUBE_METRICS_ENABLED = (env("YOUTUBE_METRICS_ENABLED", "0") or "0") == "1"
UPLOAD_INTAKE_SYNC_ENABLED = (env("UPLOAD_INTAKE_SYNC_ENABLED", "0") or "0") == "1"
UPLOAD_INTAKE_SYNC_NOT_BEFORE_MS = _int_env("UPLOAD_INTAKE_SYNC_NOT_BEFORE_MS", 0, minimum=0)
UPLOAD_INTAKE_SYNC_MAX_CREATES = _int_env("UPLOAD_INTAKE_SYNC_MAX_CREATES", 10, minimum=1)
...
# keyword generation and exceptional draft generation.  Production points the state
# path at a SQLite file on the service's /data persistent volume; /tmp JSON remains a
# safe local fallback but is not a cross-deployment daily cap.
