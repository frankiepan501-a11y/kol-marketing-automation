...
@app.post("/b2b-linkedin-daily-card/run")
async def run_b2b_linkedin_daily_card(authorization: str = Header(default=""),
    ...
    默认 dry-run 只筛选线索并返回预览; n8n 生产定时使用
    ?commit=true&notify=true 按跟进人分组发外贸助手交互卡。
    """
    _check_auth(authorization)
    try:
        ...
        tr = _tb.format_exc()[-1000:]
        await _alert_endpoint_failure("/b2b-linkedin-daily-card/run", str(e), tr)
        return {"ok": False, "error": str(e), "trace": tr}
...
@app.post("/b2b-email-outreach/run")
async def run_b2b_email_outreach(authorization: str = Header(default=""),
                                 commit: bool = False,
                                 dry_run_to: str = "",
                                 limit: int = 1,
                                 record_id: str = "",
                                 owner: str = ""):
    """B2B first-touch development email sender.

    Default is dry-run: send queued outreach samples to Frankie/test mailbox.
    Real customer sends require commit=true plus B2B_EMAIL_SEND_COMMIT=1 in env.
    """
    _check_auth(authorization)
    try:
        result = await b2b_outreach_email.run(
            commit=commit,
        ...
        tr = _tb.format_exc()[-1000:]
        await _alert_endpoint_failure("/b2b-email-outreach/run", str(e), tr)
        return {"ok": False, "error": str(e), "trace": tr}
...
async def run_b2b_linkedin_discovery(authorization: str = Header(default=""),
                                     ...
                                     market: str = "",
                                     min_score: int = 0):
    """B2B LinkedIn external discovery candidate refill.

    This only writes company-level candidate rows. It does not write LinkedIn
...
@app.post("/cs/amz-procurement-preview/send")
async def run_amz_procurement_preview_card(authorization: str = Header(default=""),
                                           mode: str = "dry_run",
                                           limit: int = 10,
                                           batch_id: str = "",
                                           record_ids: str = "",
    ...
    try:
        ...
        gray_unions = [x.strip() for x in (gray_union_ids or "").split(",") if x.strip()]
        gray_chats = [x.strip() for x in (gray_chat_ids or "").split(",") if x.strip()]
        result = await amz_procurement_preview.send_procurement_preview_card(
            mode=mode,
            limit=limit,
            batch_id=batch_id,
        ...
        tr = _tb.format_exc()[-1000:]
        await _alert_endpoint_failure("/cs/amz-procurement-preview/send", str(e), tr)
        return {"ok": False, "error": str(e), "trace": tr}
...
async def run_amz_compliance_fit_card(authorization: str = Header(default=""),
    ...
    P0 默认只发 Frankie。系统先生成风险点和建议动作；人只选择处理动作，回调只写当前候选行。
    """
    _check_auth(authorization)
    try:
        ids = [x.strip() for x in (record_ids or "").split(",") if x.strip()]
        gray_unions = [x.strip() for x in (gray_union_ids or "").split(",") if x.strip()]
        gray_chats = [x.strip() for x in (gray_chat_ids or "").split(",") if x.strip()]
        result = await amz_compliance_fit_card.send_fit_card(
            mode=mode,
            limit=limit,
            batch_id=batch_id,
            record_ids=ids or None,
...
@app.post("/cs/amz-validation50/start")
async def run_amz_validation50_start(authorization: str = Header(default=""),
                                     mode: str = "dry_run",
                                     limit: int = 10,
                                     batch_id: str = "",
                                     record_ids: str = "",
    ...
    P0 默认只发 Frankie。系统把已通过财务/采购/合规的候选写入 50 件验证进行中，并发送启动卡。
    """
    _check_auth(authorization)
    try:
        ids = [x.strip() for x in (record_ids or "").split(",") if x.strip()]
        gray_unions = [x.strip() for x in (gray_union_ids or "").split(",") if x.strip()]
        gray_chats = [x.strip() for x in (gray_chat_ids or "").split(",") if x.strip()]
        result = await amz_validation50.start_validation50(
            mode=mode,
            limit=limit,
            batch_id=batch_id,
            record_ids=ids or None,
        ...
        tr = _tb.format_exc()[-1000:]
        await _alert_endpoint_failure("/cs/amz-validation50/start", str(e), tr)
        return {"ok": False, "error": str(e), "trace": tr}
...
async def _start_launch_runtime_job(*, campaign_id: str, mode: str,
    ...
    if mode == "evidence_author_continuation":
        request_key += (
            ...
            f"|limit={int(continuation_import_limit)}"
        )
...
@app.post("/launch/outcomes/reconcile")
async def launch_outcomes_reconcile(
    request: Request, authorization: str = Header(default=""),
):
    """回填明确承诺和真实上稿事实；默认只预演，不发送邮件或卡片。"""
    _check_auth(authorization)
    payload = await _launch_json(request)
    dry_run = payload.get("dry_run", True)
    if not isinstance(dry_run, bool):
        raise HTTPException(status_code=422, detail="dry_run 必须是 JSON 布尔值")
    if not dry_run and not config.LAUNCH_ACTIVITY_QUEUE_ENABLED:
        raise HTTPException(status_code=403, detail="活动队列写入开关未开启")
    return await _run_launch_write(
        "/launch/outcomes/reconcile",
        lambda: launch_outcomes.reconcile_campaign(
            _launch_required(payload, "campaign_id"), dry_run=dry_run,
        ),
    )
