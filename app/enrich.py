...
async def enrich_task(task_record: dict, seen_kb: set = None) -> dict:
    ...
    # → 跳过整任务 + 告警, 零外发.
    # 2026-06-02: gate 放宽 — 官网链接 OR 亚马逊链接 任一非空即可(运营改用填亚马逊链接, 信息更丰富).
    if not feishu.product_url(product["fields"]):
        p_disp = ext(product["fields"].get("产品英文名")) or ext(product["fields"].get("产品名"))
        await feishu.update_record(config.T_TASK_KOL, task_rid, {
            "任务状态": "8-已取消",
            "备注": "产品缺对外链接(官网链接/亚马逊链接都为空) → cold 会带死链, 已跳过派单(零外发). 填任一链接后重跑.",
        })
        try:
            await feishu.send_card_message("chat_id", config.NOTIFY_CHAT_ID, {
                "header": {"template": "red", "title": {"tag": "plain_text",
                    "content": "🚨 KOL 派单跳过 — 产品缺对外链接(防死链)"}},
                "elements": [{"tag": "div", "text": {"tag": "lark_md",
                    "content": f"**任务**: {task_name}\n**产品**: {p_disp}\n**原因**: 产品库「官网链接」「亚马逊链接」都为空 → cold 邮件会带死链, 已跳过整批派单(零外发). 请填任一链接再重跑。"}}],
            }, biz="KOL")
        except Exception:
            pass
        return {"task": task_name, "error": "产品缺对外链接,跳过防死链", "task_rid": task_rid}

    # 产品英文名引用拼接 (2026-06-02): FUNLAB 用老库ERP SKU 反查 SKU 库自动拼+回填,
    # POWKONG 暂手填校验. 同步 product 内存值供下游生草稿用.
    naming = await product_naming.resolve_and_backfill(product)
    if naming.get("warn"):
        print(f"[enrich] 产品英文名: source={naming['source']} backfilled={naming['backfilled']} "
              f"name={naming['name']!r} WARN={naming['warn']}")

    # 护栏: 完全没有英文名 → 正文会降级成中文 → 中英混杂(Scott Stein 铁律). 拦下整批, 提示补.
    if not naming.get("name"):
        p_disp = ext(product["fields"].get("产品名"))
        await feishu.update_record(config.T_TASK_KOL, task_rid, {
            "任务状态": "8-已取消",
            "备注": "产品缺英文名(自动拼需补「老库ERP SKU」/POWKONG 需手填「产品英文名」) → 防中英混杂, 已跳过派单. 补后重跑.",
        })
        try:
            await feishu.send_card_message("chat_id", config.NOTIFY_CHAT_ID, {
                "header": {"template": "orange", "title": {"tag": "plain_text",
                    "content": "🟠 KOL 派单跳过 — 产品缺英文名(防中英混杂)"}},
                "elements": [{"tag": "div", "text": {"tag": "lark_md",
                    "content": f"**任务**: {task_name}\n**产品**: {p_disp}\n**原因**: {naming.get('warn') or '产品英文名为空'}\n**解决**: FUNLAB 在产品库填「老库ERP SKU」(系统自动拼英文名) + 「主关键词(英文)」; POWKONG 手填「产品英文名」。补后重跑。"}}],
            }, biz="KOL")
        except Exception:
            pass
        return {"task": task_name, "error": "产品缺英文名,跳过防中英混杂", "task_rid": task_rid}
