...
## 2026-09-14 P1-7：B2B 邮件提醒收件路由调整

- Frankie 已授权优先完成 P1-7；本轮只改邮件回执主卡从群发到按「邮箱负责人」私聊，不进入 P1-2 至 P1-6。
- 固定基线为 `49440689dc4222c583ebe03e97cc8d880fe9e3ff`；新分支/工作树从该生产版本创建，避免污染已完成的 P1-1 工作树。
- 生产只读预检：当前 3 位负责人在 `B2B_LINKEDIN_OWNER_NOTIFY_JSON` 中覆盖 3/3；3 个目标均为外贸助手同 namespace 的有效私聊、无群目标、无重复目标；24 小时升级目标当前有效。
- n8n `RUGz3kEIVBo6uCiW` 继续每日 10:00 调同一 endpoint，参数无需修改。本阶段未触发任务、未发卡、未写 Base。
- 当前进入测试先行：公开验收入口为 `b2b_mail_reminder.run(commit=True, notify=True)`，必须在任何真实发送前完成全批路由校验。
- 失败测试已证明旧代码仍固定向群发送，且忽略缺失、群目标、重复目标和部分失败语义；最终专项 `21 passed + 4 subtests`，B2B 相关回归 `71 passed + 6 subtests`，全仓 `1210 passed + 54 subtests`。
- 最小实现只改邮件模块、异步结果白名单、env 注释和测试：提醒表按「邮箱负责人」分组，只接受外贸助手 namespace 的 `open_id/union_id`；所有目标先验后才发送；部分失败只标记成功负责人的记录并抛错。
- 现有 24 小时吴晓丹升级私聊保持；不调用全局 CUS 群路由。既有回执路径实际是同会话文字反馈、没有 PATCH 原卡，已列 P2 而未夹带修复。
