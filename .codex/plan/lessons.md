...
## 2026-08-26 Windows嵌入式Python的全局导入污染

- 症状：在目标仓库目录直接运行嵌入式Python的`-m pytest`，仍可能优先导入另一项目的`app`包。
- 根因：该解释器的持久配置残留了外部项目路径，当前工作目录不保证排在它之前。
- 做法：本项目发布测试统一用`-c`先执行`sys.path.insert(0, <当前仓库>)`，再调用`pytest.main(...)`；看到导入路径指向别的项目时，先判为测试环境污染，不判为代码失败。
